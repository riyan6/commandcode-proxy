...
遵循现有 ESM 风格：2 个空格缩进、单引号、语句末尾使用分号，优先使用 `const`。变量和函数使用 `camelCase`，常量使用全大写下划线。协议字段和请求转换应以本地 `command-code@1.7.0` bundle 为基线；发送给上游的 CLI 版本头允许跟随 npm latest。新增或修改代码必须补充清晰的中文注释。目前没有配置 ESLint 或格式化工具，提交前请保持手工格式一致。
