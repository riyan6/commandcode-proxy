...
基于本机 `command-code@1.53.1` CLI bundle 和真实流量抓包，对齐 Command Code API 请求协议，并实现多层兼容适配。
...
| `protocolVersion` | `1.53.1` | 请求协议实现基线，同时作为 `x-command-code-version` 头发送 |
...
为保持上游信封与 1.53.1 CLI 一致，`top_p`、`stop`、`user`、`presence_penalty`、`frequency_penalty`、`response_format`、`tool_choice` 和 `parallel_tool_calls` 不会转发给上游，因此不会影响实际生成。
...
| `tool_choice` | `{type:"auto"/"any"/"tool"}` | 接受但不发送给上游，以保持 1.53.1 CLI 信封一致 |
...
代理会在相同路径透传 Command Code 原生 API。`/alpha/*`、`/provider/*` 以及 1.53.1 bundle 声明的 `/beta/*`、`/internal/*` 会被发送到固定的 `apiBase`；其他路径不会转发，因此它不是任意 URL 代理。
...
基于对本机 `command-code@1.53.1` bundle 的逐函数分析与 1.32.1 CLI 实际流量抓包，实现了以下兼容适配：

| 机制 | 实现 |
|------|------|
| **按 Key 分 Session** | 每个 API Key 独立 session，12h 过期 + 1h 随机抖动 |
| **协议基线 / 版本头** | 请求协议与 `x-command-code-version` 都固定为 `1.53.1`（`protocolVersion`）；不再跟随 npm latest，发送的版本号始终与实现一致 |
| **CLI 信封格式** | config/memory/taste/skills/permissionMode/mode/params/threadId |
| **提示词缓存** | 1.53.1 的 system 为分节数组（`toWireSystem`），参与缓存的节携带 `cache_control: {type:"ephemeral"}`；代理把调用方的系统提示作为单个缓存节发送 |
| **工具与图片格式** | 工具字段、base64 图片和 mimeType 对齐 1.53.1 wire format；tool-result 回填 toolName，历史消息中的 `tool_search` 按 CLI 规则重命名为 `search_tools` |
| **流式续接** | `pause_turn` 最多按同一请求线程继续 5 次（对齐 1.53.1 的重试上限） |
| **服务端工具结果** | 1.31.0 的 `tool-result` 事件（provider 执行）对 OpenAI/Anthropic 客户端静默跳过 |
| **上游 abort** | 1.31.0 的 `abort` 事件视为正常结束 |
| **稳定指纹** | 按 1.53.1 的加盐指纹算法（`command-code:device-fingerprint:v1` 盐 + machineId/MAC 信号哈希）生成；信号值按 API Key 派生，CPU、核心数和内存保持一致，重启后稳定 |
...
```json
{
  ...
  "params": {
    ...
    "tools": [],
    "system": [
      { "type": "text", "text": "你是一个编码助手", "cache_control": { "type": "ephemeral" } }
    ],
    "max_tokens": 64000,
    "stream": true,
...
`system` 采用 1.53.1 的分节格式（`toWireSystem`）：文本块数组，参与缓存的节携带 `cache_control: {type:"ephemeral"}`；调用方没有系统提示时整体省略。
